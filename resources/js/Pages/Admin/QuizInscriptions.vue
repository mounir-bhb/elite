<template>
    <div class="flex justify-center pt-6">
        <table>
            ...
            <tr v-for="(new_inscription, index) in new_inscriptions" @click="openValidation(new_inscription, index)" class="cursor-pointer hover:bg-slate-50">
                <td>
        ...
        <Modal :show="is_open" @close="is_open=false">
            <form @submit.prevent="submit" class="flex flex-col items-center justify-center p-4">
                <img :src="'/storage/'+form.inscription.ccps[len_ccps-1].image_url" alt="">
        ...
        methods: {
            ...
            submit(){
                if(this.form.wrong_msg){
                    this.form.post(route('quiz.inscription.refus'), {
                        preserveScroll: true
                    })
                }
                else{
                    this.form.post(route('quiz.inscription.validation'), {
                        preserveScroll: true
                    })
                }
