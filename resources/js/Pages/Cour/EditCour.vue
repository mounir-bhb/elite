<template>
    <div>
        <form @submit.prevent="submit" class="flex flex-col items-center p-4">
            <h2 class="text-center">Modifier le cour {{ cour.titre }}</h2>
            <div>
                <div>
                ...
                <div v-if="!form.videoId">
                    <video :src="'/storage/'+cour.video.video_url" type="video/mp4" class="w-20"></video>
                </div>
                <div class="mt-3">
...
<script>
    ...
    NewVideo
        ...
        data(){
            return {
                form: this.$inertia.form({
                    _method:'put',
                    titre: this.cour.titre,
                    resume: this.cour.resume,
                    chapitre: this.cour.chapitre_id,
                    //video_url: '',
                    gratuit: this.cour.gratuit,
                    //pdf_files:[],
                    videoId: null,
                    //can_submit: false
                })
        ...
        methods:{
            submit() {
                //console.log(this.form );
                this.form.post(route('cour.update', this.cour.id), {
                    preserveScroll: true
                })
                this.$emit('close')
            }
